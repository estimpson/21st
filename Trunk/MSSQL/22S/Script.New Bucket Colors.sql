...
from
	(	select
			old_part = p.part
		,	part = substring(p.part, 1, len(p.part) - 1) + mcl.ColorCode
...
where
	not exists
		(	select
			...
			where
				pm2.part = pmNew.part
				and pm2.sequence = pmNew.sequence
		)
order by
	pmNew.part
...
from
	(	select
			old_part = p.part
		,	parent_part = substring(p.part, 1, len(p.part) - 1) + mcl.ColorCode
		,	ar.sequence
		,   ar.code
		,   part = substring(p.part, 1, len(p.part) - 1) + mcl.ColorCode
		,	ar.notes
...
--				*
--			from
--				FT.New22_Buckets_bill_of_material_ec_base_material nbbomebm
--			where
--				nbbomebm.parent_part = bome.parent_part
--				and nbbomebm.part = bome.part
--		)

--drop table
--	FT.New22_Buckets_bill_of_material_ec_base_material
...
,   bomNew.unit_measure
,   bomNew.reference_no
,	std_qty = bomNew.tot_qty * (1 - bomNew.LetDownRate)
,   bomNew.scrap_factor
...
from
	(	select
		...
		,	tot_qty =
				case
					when p.part = '120019B' then 3.64
					when p.part = '120020B' then 3.79
					when p.part = '120021B' then 3.83
				end
				--(	select
		 	--			sum(bom2.std_qty)
		 	--		from
		 	--			dbo.bill_of_material_ec bom2
				--	where
				--		bom2.end_datetime is null
				--		and bom2.parent_part = p.part
				--		and bom2.substitute_part = 'N'				
		 	--	)
		,	mcl.LetDownRate
		from
			dbo.part p
...
,   bomNew.unit_measure
,   bomNew.reference_no
,	std_qty = bomNew.tot_qty * (bomNew.LetDownRate)
,   bomNew.scrap_factor
,   bomNew.engineering_level
,   bomNew.operator
,   bomNew.substitute_part
...
,   bomNew.note
--into
--	FT.New22_Buckets_bill_of_material_ec_colorant
from
	(	select
			old_part = p.part
		,	parent_part = substring(p.part, 1, len(p.part) - 1) + mcl.ColorCode
...
,   bomNew.note
--into
--	FT.New22_Buckets_bill_of_material_ec_recessed
from
	(	select
			old_part = p.part
		,	parent_part = p.part
		,	part = left(p.part, 4) + substring(p.part, 6, len(p.part))
		,   type = 'M'
		,   quantity = 1
		,   unit_measure = 'EA'
		,   reference_no = null
		...
		from
			...
			join custom.MoldingColorLetdown mcl
				on mcl.MoldApplication = '22-Bucket'
		where
			p.part like '1200R[12][901]' + mcl.ColorCode
	) bomNew
where
	not exists
		(	select
