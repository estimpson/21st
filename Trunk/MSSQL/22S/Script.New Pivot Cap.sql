...
,	pNew.low_level_code
--into
--	FT.New22_PivotCaps_part
from
	(	select
			old_part = p.part
		,	part = substring(p.part, 1, len(p.part) - 3) + mcl.ColorCode
		,	name = mcl.ColorName + ' ' + p.name
		,	p.cross_ref
		,	p.class
		,	p.type
		,	p.commodity
...
--into
--	FT.New22_PivotCaps_part_inventory
from
	(	select
			old_part = p.part
		,	part = substring(p.part, 1, len(p.part) - 3) + mcl.ColorCode
...
--into
--	FT.New22_PivotCaps_part_standard
from
	(	select
			old_part = p.part
		,	part = substring(p.part, 1, len(p.part) - 3) + mcl.ColorCode
...
,   pmNew.crew_size
--into
--	FT.New22_PivotCaps_part_machine
from
	(	select
			old_part = p.part
		,	part = substring(p.part, 1, len(p.part) - 3) + mcl.ColorCode
...
,   arNew.time_stamp
--into
--	FT.New22_PivotCaps_activity_router
from
	(	select
			old_part = p.part
		,	parent_part = substring(p.part, 1, len(p.part) - 3) + mcl.ColorCode
		,	ar.sequence
		,   ar.code
		,   part = substring(p.part, 1, len(p.part) - 3) + mcl.ColorCode
		,	ar.notes
