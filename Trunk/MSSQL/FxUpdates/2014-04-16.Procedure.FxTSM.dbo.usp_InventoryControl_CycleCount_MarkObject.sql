...
/*
Create Procedure.FxTSM.dbo.usp_InventoryControl_CycleCount_MarkObject.sql
*/

--use FxTSM
--go

if	objectproperty(object_id('dbo.usp_InventoryControl_CycleCount_MarkObject'), 'IsProcedure') = 1 begin
	drop procedure dbo.usp_InventoryControl_CycleCount_MarkObject
end
go

create procedure dbo.usp_InventoryControl_CycleCount_MarkObject
	@User varchar(10)
...
,	CorrectedQuantity =
		case
			when @NewStatus < 0 then null
			else iccco.CorrectedQuantity
		end
,	CorrectedLocation = 
		case
			when @NewStatus < 0 then null
			else iccco.CorrectedLocation
		end
from
	dbo.InventoryControl_CycleCountObjects iccco
	join #serialList sl
		on sl.serial = iccco.Serial
where
	iccco.RowCommittedDT is null
