alter procedure dbo.usp_ReceivingDock_ReceiveObjects_againstLineItem
(	@User varchar(5)
,	@PONumber integer
,	@PartCode varchar(25)
...
where
	...
	(	select
		...
		where
			po_number = @PONumber
			and part_number = @PartCode
			and status = 'A'
			and	balance > 0), pd.date_due)
...
set
	@Conversion = coalesce(
	(	select
			min(unit_conversion.conversion)
		from
			po_detail
		...
		where
			...
			and po_detail.date_due = @DueDT), 1)
