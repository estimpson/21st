...
[LastUser] [sys].[sysname] NOT NULL CONSTRAINT [DF__bill_of_m__LastU__1432B864] DEFAULT (suser_sname()),
[LastDT] [datetime] NULL CONSTRAINT [DF__bill_of_m__LastD__1526DC9D] DEFAULT (getdate()),
		...
		where
			exists
				(	select
					...
					where
						bome.ID != i.ID
						and bome.parent_part = i.parent_part
						and bome.part = i.part
						and
							(	bome.end_datetime is null
								or bome.end_datetime > getdate()
							)
				)
	) begin
...
GO
ALTER TABLE [dbo].[bill_of_material_ec] ADD CONSTRAINT [PK__bill_of_material__124A6FF2] PRIMARY KEY CLUSTERED  ([parent_part], [part], [start_datetime]) ON [PRIMARY]
GO
ALTER TABLE [dbo].[bill_of_material_ec] ADD CONSTRAINT [UQ__bill_of_material__133E942B] UNIQUE NONCLUSTERED  ([ID]) ON [PRIMARY]
GO
