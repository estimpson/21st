...
CREATE VIEW [dbo].[vr_ipeds_detail] AS

-- New hire reports include employees hired during the reporting period
-- even if they terminated during the reporting period.  The other reports
-- don't include terminated employees.  This view excludes the employees who
...
WHERE
	(vr_ipeds_detail_with_new_hires.termination_date IS NULL OR
	vr_ipeds_detail_with_new_hires.termination_date > CONVERT(DATETIME, CONVERT(VARCHAR(4), YEAR(vr_ipeds_detail_with_new_hires.ipeds_date)) + '1031'))
GO
