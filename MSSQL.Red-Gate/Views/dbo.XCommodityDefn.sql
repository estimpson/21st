...
create trigger [dbo].[tr_XCommodityDefn_iud] on [dbo].[XCommodityDefn] instead of insert, update, delete
as
insert
	dbo.CommodityDefn
(	ParentCommodityID
...
where
	...
	(	select
		...
		from
			deleted
		where
			inserted.TopCommodityID = deleted.TopCommodityID
		and
			inserted.Sequence = deleted.Sequence) and
	not exists
	(	select
			*
		from
			dbo.CommodityDefn cd
		where
			cd.CommodityID = inserted.CommodityID)

update
	dbo.CommodityDefn
set
	ParentCommodityID = inserted.ParentCommodityID
,	DrAccount = inserted.DrAccount
from
	dbo.CommodityDefn cd
	join inserted on
		cd.CommodityID = inserted.CommodityID
	join deleted on
		inserted.TopCommodityID = deleted.TopCommodityID
	and
		inserted.Sequence = deleted.Sequence
GO
