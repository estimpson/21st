...
--01/25/2017 asb FT, LLC -- added code to ignore 52s and 45s part numbers
...
declare @IgnoreShippers table -- asb 01/25/2017 ignoring 52s and 45s part number ASNs; now ignoring if ASNLineview returns no rows ( asb 08/15/2017)
...
Insert @IgnoreShippers

 SELECT s.id, 
		coalesce((select count(distinct customer_part) from shipper_detail  sd where sd.shipper = s.id and (sd.customer_part  like '%52S-%' or  sd.customer_part  like '%45S-%' )),0)  ,
	...
	UNION
		...
		Join
			edi_setups es on es.destination = s.destination and es.asn_overlay_group = 'PLX'
			where s.date_shipped >getdate()-180 
			and not exists ( Select 1 from [EDI_XML_PLEX_ASN].[ASNLines] where ShipperID = s.id )
...
if	@ShipperList > '' begin
	...
	where
		coalesce(s.type, 'N') = 'N'
		and s.status = 'C'
		and s.date_shipped > getdate() - 8 and 
		s.id not in
			(	Select
					shipperID
				from
...
declare
	@ShipperList varchar(max) = '76096, 76023'
