...
--- <Update rows="*">
set	@TableName = 'EDI4010.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI4010.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI4010.ShipSchedules', 'Status', 'Replaced'))
		end
from
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDI4010.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI4010.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDI4010.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI4010.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI4010.PlanningHeaders', 'Status', 'Replaced'))
	end
from
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI4010.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI4010.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDI4010.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDI4010.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDI4010.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI4010.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI4010.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI4010.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI4010.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI4010.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDI4010.ShipScheduleHeaders fh
...
from
	EDI4010.ShipScheduleHeaders fh
	join EDI4010.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI4010.ShipScheduleAccums fa
	...
	left join EDI4010.ShipScheduleAuthAccums faa
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDI4010.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDI4010.PlanningHeaders fh
	join EDI4010.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI4010.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDI4010.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDI4010.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDI4010.PlanningReleases', 'Status', 'Active'))
...
if	@Testing = 0 begin
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDI4010.PlanningHeaders fh
					join EDI4010.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDI4010.ShipScheduleHeaders fh
					join EDI4010.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDI4010.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDI4010.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDI4010.ShipSchedules b
 Join 
	EDI4010.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDI4010.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDI4010.ShipScheduleAccums ssa on 
 	...
 	left join
		EDI4010.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	...
	left join
 	...
 	left join
		EDI4010.PlanningAuthAccums praa on 
...
Begin
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDI4010' 
