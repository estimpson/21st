...
create procedure [dbo].[usp_InventoryControl_Transfer_ObjectToPallet]
	@Operator varchar(5)
,	@Serial int
,   @PalletSerial int
...
-- Is not staged to ship.
if not exists
	(	select	1
		from	object
		where	serial = @Serial
				and shipper is null) begin
				
	set	@Result = 60004
	rollback tran @ProcName
	RAISERROR ('Error in procedure %s. Serial %d is on a shipper.', 16, 1, @ProcName, @Serial)
	return	@Result
end

-- Pallet is not staged to ship.
if not exists
	(	select	1
		from	object
	...
	set	@Result = 60004
	rollback tran @ProcName
	RAISERROR ('Error in procedure %s. Pallet %d is on a shipper.', 16, 1, @ProcName, @PalletSerial)
	return	@Result
end

-- Is not already on the pallet.
if exists
	(	select	1
		from	object
		where	serial = @Serial
				and object.parent_serial = @PalletSerial) begin

	set	@Result = 60001
	rollback tran @ProcName
	RAISERROR ('Box %d is already on pallet %d.', 16, 1, @Serial, @PalletSerial)
	return	@Result
end
---	</ArgumentValidation>
...
declare @PalletOriginal int
select
	@PalletOriginal = o.parent_serial
from
	object o
where
	o.serial = @Serial
...
--- <Update object's location and parent serial>
set	@TableName = 'dbo.object'
update 
	o
set
	o.location = oPlt.location
,	o.parent_serial = @PalletSerial
from
	object o
	join dbo.object oPlt
		on oPlt.serial = @PalletSerial
where
	o.serial = @Serial
...
,   po_number = ''
,   operator = @Operator
,   from_loc = coalesce(convert(varchar, @PalletOriginal), 'None')
,   to_loc = convert(varchar, o.parent_serial)
...
,   shipper =
		case
			when o.shipper is not null then convert(varchar(20), o.shipper)
			else o.shipper
		end
,   flag = ''
,   activity = ''
