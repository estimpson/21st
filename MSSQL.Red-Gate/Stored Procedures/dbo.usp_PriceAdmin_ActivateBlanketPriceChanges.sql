...
if @AllowUpdate = 1 begin
	...
	select
			...
			NewCustomerPO = coalesce(nullif(bpc.CustomerPO,''), order_header.customer_po) ,
			UserCode = @UserCode,
			UserName = @UserName,
			ActivatedDate = @ActivatedDate
	from	
	...
	set		
			...
			order_header.customer_po = coalesce(nullif(bpc.CustomerPO,''), order_header.customer_po)
	from	
			order_header join
			BlanketPriceChanges bpc on 
			bpc.Part = order_header.blanket_part and
...
if @AllowUpdate = 1 begin
	...
	select
			...
			NewCustomerPO = coalesce(nullif(bpc.CustomerPO,''), shipper_detail.customer_po) ,
			UserCode = @UserCode,
			UserName = @UserName,
			ActivatedDate = @ActivatedDate
	from	
	...
	set		
			...
			shipper_detail.customer_po = coalesce(nullif(bpc.CustomerPO,''), shipper_detail.customer_po)
	from	
			shipper_detail join
			shipper on shipper_detail.shipper = shipper.id join
			order_header on
