...
CREATE PROCEDURE [FT].[Custom_Pricing_UpdateIrwinPricing_includeTelescopic]
AS
...
SELECT Tp.part,
			CASE WHEN tp.part like 'P%' THEN 'Telescopic' ELSE 'IRWIN-GRAN' END,
			tp.Part,
			piv.standard_pack,
			'EA',
			'B',
...
WHERE  NOT EXISTS ( SELECT 1 FROM dbo.part_customer pc WHERE pc.customer in ('IRWIN-GRAN', 'Telescopic') AND pc.part = tp.part)


--update part customer 
UPDATE pc
SET pc.blanket_price =  ROUND(lastprice.Price,2)
FROM
	part_customer pc
...
WHERE pc.customer in ('IRWIN-GRAN', 'Telescopic')



--update order_header
UPDATE oh
SET		oh.price = ROUND(lastprice.Price,2)
		,	oh.alternate_price = ROUND(lastprice.price,2)
FROM
	order_header oh
...
WHERE oh.customer in ('IRWIN-GRAN', 'Telescopic') AND ROUND(lastprice.price,2) != oh.alternate_price


--update order_detail

UPDATE od
SET od.price = ROUND(lastprice.Price,2),	
	od.alternate_price = ROUND(lastprice.Price,2)
...
WHERE EXISTS ( SELECT 1 FROM order_header WHERE order_header.order_no = od.order_no AND order_header.customer in ('IRWIN-GRAN', 'Telescopic'))

--Update shipper_detail FOR SHIPPERS ON DOCK

UPDATE sd
SET sd.price = ROUND(lastprice.Price,2),	
	sd.alternate_price = ROUND(lastprice.Price,2)
...
WHERE EXISTS ( SELECT 1 FROM shipper WHERE shipper.id = sd.shipper AND shipper.customer in ('IRWIN-GRAN', 'Telescopic') AND shipper.date_shipped IS NULL AND shipper.TYPE IS NULL AND SHIPPER.STATUS IN ('O', 'S'))
...
FROM	
	dbo.shipper_detail sd
CROSS APPLY ( SELECT TOP 1 * FROM dbo.Temp_IRWIN_Pricing tp2 WHERE tp2.Part =sd.part_original AND tp2.EffectiveDate<= GETDATE() ORDER BY tp2.EffectiveDate DESC ) lastprice
WHERE EXISTS ( SELECT 1 FROM shipper WHERE shipper.id = sd.shipper AND shipper.customer in ('IRWIN-GRAN', 'Telescopic') AND shipper.date_shipped >= lastprice.EffectiveDate AND shipper.TYPE IS NULL AND SHIPPER.STATUS IN ('C', 'Z') AND invoice_printed ='N')
